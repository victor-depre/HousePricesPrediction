...
library(naniar) # pour trouver l'emplacement des NA
library(Boruta) # pour trouver les variables importantes
```
...
On peut voir que train.df contient 82 variables.

```{r}
#str(test.df)
```

test.df contient seulement 81 variables, donc une de moins que train.df, c'est normal il s'agit de la variable SalePrice qui doit être déterminée.

```{r}
test.df$SalePrice <- NA
```

On attribue donc la valeur NA pour la variable SalePrice dans le dataset test.df.

```{r}
full.df <- rbind(train.df, test.df)
```

On merge les deux datasets pour pouvoir analyser la totalité des données.
